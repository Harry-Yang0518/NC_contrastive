...
def get_args_parser():
    ...
    parser.add_argument('--out_dim', default=65536, type=int, help="""Dimensionality of
        the DINO head output. For complex and large datasets large values (like 65k) work well.""") #32/64/128/256
    ...
    parser.add_argument('--mixup_alpha', default=0.8, type=float, help="""Alpha parameter for the beta
        distribution used for MixUp augmentation. Set to 0 to disable MixUp.""")
    ...
    parser.add_argument('--weight_decay', type=float, default=0.05, help="""Initial value of the
    ...
    parser.add_argument('--batch_size_per_gpu', default=768, type=int,
    ...
    parser.add_argument('--local_crops_scale', type=float, nargs='+', default=(0.4, 0.8),
        help="""Scale range of the cropped image before resizing, relatively to the origin image.
    ...
    # Misc
    parser.add_argument('--store_name', default='test', type=str, help='Path to save logs and checkpoints.')
...
def train_one_epoch(student, teacher, dino_loss, data_loader, optimizer, lr_schedule, wd_schedule, momentum_schedule, epoch, fp16_scaler, args, alpha=1.0):
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        # Apply MixUp
        mixed_images, labels_a, labels_b, lam = mixup_data(images[0], labels, args.mixup_alpha)

        # teacher and student forward passes + compute dino loss
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            teacher_output = teacher(mixed_images[:2])  # only the 2 global views pass through the teacher
        ...
        # MixUp criterion
        loss = lam * F.cross_entropy(student_output, labels_a) + (1 - lam) * F.cross_entropy(student_output, labels_b)
